...
const destinations = [
  ...
  { title: "Tests", description: "Find a run and inspect its results", href: "/runs" },
...
export function CommandMenu() {
  ...
  return <Dialog.Root open={open} onOpenChange={value => { setOpen(value); setQuery(""); }}>
    ...
    <Dialog.Portal><Dialog.Overlay className="dialog-overlay" /><Dialog.Content className="command-dialog">
      <div className="dialog-heading"><Dialog.Title>Quick navigation</Dialog.Title><Dialog.Close className="icon-button" aria-label="Close quick navigation"><X size={20} /></Dialog.Close></div>
      ...
      <Input id="command-query" placeholder="Search pages…" value={query} onChange={event => setQuery(event.target.value)} autoComplete="off" />
