...
export function FindingListItem({
  ...
  const referenceCount =
    finding.evidence.step_ids.length + finding.evidence.artifact_ids.length;
  return (
    <li className="finding-item">
      <Link to={`/runs/${runId}/findings/${finding.id}`}>
        ...
        <div className="finding-copy">
          ...
          <div className="finding-meta">
            <span>
              ...
              {finding.journey_ids.length} affected{" "}
              {finding.journey_ids.length === 1 ? "journey" : "journeys"}
            </span>
            <span>
