...
export function TargetsPage() {
  ...
  const { applicationId = "" } = useParams(); const { workspace } = useOutletContext<AppShellContext>(); const appState = useAsync(() => api.getApplication(applicationId), [applicationId]); const targetsState = useAsync(() => api.listTargets(applicationId), [applicationId, revision]);
  ...
  return <><PageHeader eyebrow={appState.data.name} title="Targets" description="Environments MarketTwin may test after explicit authorization." action={canWrite ? <Link className="primary-button" to={`/applications/${applicationId}/targets/new`}><Icon name="plus" size={16} /> Add target</Link> : undefined} />{targetsState.status === "loading" ? <LoadingPanel label="Loading targets" /> : targetsState.status === "error" ? <ErrorPanel message={targetsState.error} /> : targetsState.data.length === 0 ? <EmptyState title="No targets configured" copy="Add an environment before creating a test." action={canWrite ? <Link className="primary-button" to={`/applications/${applicationId}/targets/new`}>Add target</Link> : undefined} /> : <div className="data-list">{targetsState.data.map((target) => <div className="deletable-row" key={target.id}><Link className="data-row" to={`/targets/${target.id}`}><span className="row-icon"><Icon name="target" size={17} /></span><div className="row-primary"><strong>{target.name}</strong><span>{target.base_url}</span></div><span className="environment-label">{target.environment}</span><Icon name="arrow" size={16} /></Link>{canManageLifecycle(workspace.role) ? <DeleteAction kind="target" id={target.id} name={target.name} onDeleted={() => { setRevision(value => value + 1); }} /> : null}</div>)}</div>}</>;
}
