...
export function NewTargetPage() {
  ...
  if (!canWriteWorkspace(workspace.role))
    return (
      <>
        <PageHeader eyebrow="Target" title="Add environment" />
        <div className="error-panel" role="alert">
          <div>
            <strong>Read-only workspace</strong>
            <span>Your workspace role cannot create targets.</span>
          </div>
        </div>
      </>
    );
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    try {
      const target = await api.createTarget(applicationId, {
        name: name.trim(),
        environment,
  ...
  return (
    <>
      <PageHeader
        eyebrow="Application target"
        title="Add environment"
        description="MarketTwin derives the initial allowed origin from the base URL. Additional network origins stay blocked until explicitly supported."
      />
      <div className="two-column-layout">
        <form className="panel form-grid" onSubmit={submit}>
          ...
          <label className="toggle-row">
            <span>
              ...
              <small>
                This target requires login or another authentication step.
              </small>
            </span>
            <input
              type="checkbox"
        ...
        <aside className="info-card">
          ...
          <h2>Network policy stays explicit</h2>
          <p>
            The browser runtime validates the target origin and request
            destinations. This form never stores target credentials.
          </p>
        </aside>
      </div>
    </>
