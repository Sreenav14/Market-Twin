...
export function TargetOverviewPage() {
  ...
  if (
    ...
    return <LoadingPanel label="Loading target" />;
  ...
  return (
    <>
      <PageHeader
        eyebrow="Target"
        title={target.name}
        description={target.base_url}
        action={
          authorized && canWrite ? (
            <Link
              className="primary-button"
              to={`/applications/${target.application_id}/runs/new`}
            >
            ...
            >
              <Icon name="shield" size={16} /> Authorization
            </Link>
          )
        }
      />
      <div className="detail-grid">
        <section className="panel">
          ...
          <dl className="definition-list">
            <div>
              <dt>Environment</dt>
              <dd>{target.environment}</dd>
            </div>
            <div>
              <dt>Authentication</dt>
              <dd>{target.requires_auth ? "Protected" : "Public"}</dd>
            </div>
            <div>
              <dt>Authorization</dt>
              <dd>
            ...
            <div>
              <dt>Base URL</dt>
              <dd className="monospace">{target.base_url}</dd>
            </div>
          </dl>
        </section>
        <section className="panel">
          ...
          <div className="origin-list">
            {target.allowed_origins.map((origin) => (
              <div
                key={`${origin.scheme}-${origin.hostname}-${origin.port ?? "default"}`}
              >
                <code>
          ...
          <p className="panel-note">
            Additional origin management needs a dedicated Control API. The UI
            will not offer an unsafe “allow all” shortcut.
          </p>
        </section>
      </div>
      {canManageLifecycle(workspace.role) ? (
        <LifecyclePanel
          ...
          entityId={target.id}
          returnTo={`/applications/${target.application_id}/targets`}
        />
      ) : null}
    </>
  );
