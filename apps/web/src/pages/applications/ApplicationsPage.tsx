...
export function ApplicationsPage() {
  ...
  const state = useAsync(
    () => api.listApplications(workspace.id),
    ...
    <>
      ...
      {state.status === "loading" ? (
        ...
        <EmptyState
          ...
          copy="Create an application to organize targets and tests."
          action={
            canWrite ? (
              <Link className="primary-button" to="/applications/new">
                Create application
        ...
        <div className="data-list" role="list">
          {state.data.map((application) => (
            <div className="deletable-row" key={application.id} role="listitem">
              <Link to={`/applications/${application.id}`} className="data-row">
                <span className="product-icon small">
                  {application.name.slice(0, 2).toUpperCase()}
                </span>
                <div className="row-primary">
                  <strong>{application.name}</strong>
                  <span>{application.description || "No description"}</span>
                </div>
