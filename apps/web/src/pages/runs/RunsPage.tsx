...
import { canDeleteTest } from "../../lib/results";
...
export function RunsPage() {
  ...
  const runs = (state.data?.runs || []).filter(run => (status === "all" || run.status === status) && testBrief(run.configuration_snapshot).toLowerCase().includes(search.toLowerCase()));
  ...
  return <><PageHeader title="Tests" description="Track your product tests, review usability issues, and see what needs attention." action={canWriteWorkspace(workspace.role) ? <Link className="primary-button" to="/applications">New test</Link> : undefined} />
    ...
    {runs.length ? <div className="study-table"><div className="study-table-heading"><span>Test / target</span><span>Status</span></div>{runs.map(run => <div className="deletable-row" key={run.id}><Link className="study-row" to={`/runs/${run.id}/overview`}><span className="study-row-copy"><strong>{testBrief(run.configuration_snapshot)}</strong><span>{textValue(run.target_snapshot.name, "Target")}<span className="monospace">{run.id.slice(0, 8)}</span></span></span><TestStatusBadge status={run.status} /><ArrowUpRight size={18} aria-hidden="true" /></Link>{canManageLifecycle(workspace.role) ? <DeleteAction kind="test" id={run.id} name={testBrief(run.configuration_snapshot)} disabled={!canDeleteTest(run.status)} onDeleted={() => { void state.refetch(); }} /> : null}</div>)}</div> : <EmptyState title={state.data.runs.length ? "No matching tests" : "Your first test starts with a question"} copy={state.data.runs.length ? "Try another search or clear your filters." : "Choose an application and describe the experience you want to investigate."} action={state.data.runs.length ? <Button variant="secondary" onClick={() => setParams({})}>Clear filters</Button> : <Link className="primary-button" to="/applications">Choose application</Link>} />}
    </>}
  </>;
}
