...
> **Primary constraint:** Build a real agentic, microservices-based product-testing platform while keeping V1 AWS infrastructure cost as low as practical.  
> **Deployment model:** Local-first development, Docker Compose for the first vertical slice, K3s/Kubernetes before AWS deployment, and one low-cost AWS EC2 node for the initial hosted V1.

---

## Document purpose

This document is the implementation contract for MarketTwin V1. It freezes the product model, service boundaries, data ownership, core workflows, authentication design, event contracts, AWS deployment model, security requirements, cost controls, and V1 acceptance criteria.

It is intentionally much more detailed than a normal architecture overview. The purpose is to prevent major architecture drift after implementation begins. Some implementation details will still be tuned—such as model selection, prompt wording, extraction thresholds, exact memory limits, and minor dependency versions—but those changes must remain inside the boundaries defined here.
